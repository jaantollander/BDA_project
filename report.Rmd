...
We give the scores an adjustable prior because it has a scale that is common to each platform, that is, a value between 0 and 100. We chose an informative prior mean of $70$ for the scores, which is reasonably close to the sample mean for `Critic_Score` of `r mean(data_models$Critic_Score)`, which we can see in the histogram above. We set the prior standard deviation to $0.2$ to represent a slight amount of ignorance. We are saying that our best prior guess for the `Critic_Score` mean could be off by a factor of $\exp(0.2)=1.2$. [@Gelman2016]
...
```{r}
#print(separate_fit, pars = c("mu_sales", "mu_scores"))
```
...
We extract posterior draws of sales and scores for different platforms, compute the ratio of sales to scores and visualize the results. The ratio is defined as
...
p4 <- plot_posterior_for_platform("DS")
```
...
p5 <- ggplot(df, aes(x=platforms, y=variances)) + geom_col()
```
...
Let's compare the fitted posterior to the observed data in the dataset.
...
observed_data <- filter(
  ...
  Platform != "N64", Platform != "DC", Platform != "GB", Platform != "WS") %>%
  group_by(Platform) %>%
  dplyr::summarize(Ratio_Variance=var(Global_Sales / (Critic_Score / 10)))
```

```{r}
n <- length(platforms)
variances_observed <- rep(NULL, n)
for (i in 1:n) {
  variances_observed[i] <- observed_data[i,2]
}
df_observed <- data.frame(platforms=platforms, variances_observed=variances_observed)
p5_observed <- ggplot(df_observed, aes(x=platforms, y=variances_observed)) + geom_col()
```

```{r}
grid.arrange(p5 + xlab("Fitted variance"), p5_observed + xlab("Dataset variance"), ncol = 1)
```

The fitted variance for the sales to critic scores ratio follows the dataset variance for the most part. The platforms `Wii` and `X360` are the biggest differences. This can be visualized by dropping them from the barplots:

```{r}
variances_observed_drop <- variances_observed[-9][-10]
variances_fitted_drop <- variances[-9][-10]
df_observed_drop <- data.frame(platforms=platforms[-9][-10], variances_observed_drop=variances_observed_drop)
p5_observed_drop <- ggplot(df_observed_drop, aes(x=platforms, y=variances_observed_drop)) + geom_col()

df_fitted_drop <- data.frame(platforms=platforms[-9][-10], variances_fitted_drop=variances_fitted_drop)
p5_fitted_drop <- ggplot(df_fitted_drop, aes(x=platforms, y=variances_fitted_drop)) + geom_col()
grid.arrange(p5_fitted_drop + xlab("Fitted variance"), p5_observed_drop + xlab("Dataset variance"), ncol = 1)
```
...
```{r, echo = FALSE}
n = length(platforms)
variances_unif <- rep(NULL, n)
for (i in 1:n) {
  ratios_unif <- mu_sales_unif[,i] / mu_scores_unif[,i]
  variances_unif[i] <- var(ratios_unif)
}
df <- data.frame(platforms=platforms, variances=variances_unif)
p5_unif <- ggplot(df, aes(x=platforms, y=variances)) + geom_col() + xlab("uniform")
```

```{r, echo = FALSE}
n = length(platforms)
variances_alt <- rep(NULL, n)
for (i in 1:n) {
  ratios_alt <- mu_sales_alt[,i] / mu_scores_alt[,i]
  variances_alt[i] <- var(ratios_alt)
}
df <- data.frame(platforms=platforms, variances=variances_alt)
p5_alt <- ggplot(df, aes(x=platforms, y=variances)) + geom_col() + xlab("mean 50 std 0.15")
```
