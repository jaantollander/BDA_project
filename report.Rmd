...
In this analysis, we are interested in games released between the years 2009 - 2016. Also, we focused on the most popular platform in our analysis and filtered the less popular ones. 
...
```{r, echo=FALSE}
d4 <- count(data, Rating)
s4 <- count(data_scored, Rating)
g4 <- ggplot() + 
  geom_col(aes(x = reorder(Rating, -n), y = n), d4) +
  geom_col(aes(x = reorder(Rating, -n), y = n), s4, fill = "red") +
...
We use three models in this report; a separate model, a pooled model, and a hierarchical model. In the separate model, we represent each platform by a separate model with a standard deviation of $\sigma_j$. In the pooled model, we treat all platforms as equal -- the whole dataset shares a single mean $\mu$ and standard deviation $\sigma$. In the hierarchical model, we formulate a model where each group represents a single video game platform and uses a shared standard deviation $\sigma$ between all the platforms. Each model finds fits for both video game sales and critic scores. We have removed unpopular platforms, `N64`, `DC`, `GB`, `WS`, from this analysis.
...
## Model selection
We use the PSIS-LOO -method to asses the predictive performance of the pooled, separate and hierarchical Gaussian models in the video game sales dataset. Computations are performed on the R PSIS-LOO implementation, package `loo`.
...
We can see that different platforms have more variance when it comes to the relationship between sales and critic scores. In particular, the platforms developed by Nintendo have more variance, which could indicate that the consumers for these platforms care less about critic opinions. This finding could be explained due to their age ratings since platforms like Wii and DS have a lot more games made for people of all ages in comparison to a platform like PS3, which we demonstrate in the histograms below.
