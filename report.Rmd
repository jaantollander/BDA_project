...
```{r}
grid.arrange(
  p5 + xlab("Fitted variance"), 
  p5_observed + xlab("Dataset variance"), 
  ncol = 1)
```
...
df_observed_drop <- data.frame(
  ...
  variances_observed_drop=variances_observed_drop)
p5_observed_drop <- ggplot(
  df_observed_drop, 
  aes(x=platforms, y=variances_observed_drop)) + geom_col()

df_fitted_drop <- data.frame(
  platforms=platforms[-9][-10], 
  variances_fitted_drop=variances_fitted_drop)
p5_fitted_drop <- ggplot(
  df_fitted_drop, 
  aes(x=platforms, y=variances_fitted_drop)) + geom_col()
grid.arrange(
  p5_fitted_drop + xlab("Fitted variance"), 
  p5_observed_drop + xlab("Dataset variance"), 
  ncol = 1)
```
